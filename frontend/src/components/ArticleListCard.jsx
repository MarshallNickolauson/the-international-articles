...
const ArticleListCard = ({ article }) => {
    ...
    return (
        <div className={`card-shadow rounded-[16px] hover:cursor-pointer transition-all duration-200 ${isDarkMode ? 'bg-_303030' : ''}`} onClick={() => (location.pathname.includes('/my-articles') ? navigate(`/article/${article._id}/edit`) : navigate(`/article/${article._id}`))}>
                ...
                <div className='flex items-center w-full overflow-hidden'>
                    <div className='flex flex-col space-y-1 w-full'>
                        ...
                        <div className='flex space-x-3'>
                            <h1 className={`font-bold underline inline-block pb-1 transition-all duration-200 ${isDarkMode ? 'text-white hover:text-blue-300' : 'text-darkExpansion hover:text-mainBlue'} ${location.pathname.includes('/my-articles') ? 'text-xl' : 'text-lg'}`}>{location.pathname.includes('/my-articles') ? 'Edit' : 'Read More'}</h1>
                            {location.pathname.includes('/my-articles') && (
                                <h1
                                    className={`text-xl font-bold truncate transition-all duration-200 ${isDarkMode ? 'text-white' : 'text-darkExpansion'}
                                    px-3 bg-opacity-60 rounded-[16px] items-center
